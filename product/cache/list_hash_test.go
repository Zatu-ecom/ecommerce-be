...
func TestCanonicalHash_PermutationStable(t *testing.T) {
	a := CanonicalHash(map[string]string{"a": "1", "b": "2", "c": "3"})
	b := CanonicalHash(map[string]string{"c": "3", "a": "1", "b": "2"})
	if a != b {
...
func TestCanonicalHash_Sensitive(t *testing.T) {
	base := CanonicalHash(map[string]string{"page": "1", "limit": "20"})
	if got := CanonicalHash(map[string]string{"page": "2", "limit": "20"}); got == base {
		t.Fatal("page change must change the hash")
	}
	if got := CanonicalHash(map[string]string{"page": "1"}); got == base {
		t.Fatal("dropped param must change the hash")
	}
	if got := CanonicalHash(map[string]string{"page": "1", "limit": "20", "x": ""}); got == base {
...
func TestListKey_Shape(t *testing.T) {
	k, err := ListKey(7, FamilyProductList, "3", "abc123")
	if err != nil {
		t.Fatal(err)
	}
	...
	ck, err := ListKey(7, FamilyCollectionList, "0", "abc123")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := ListKey(0, FamilyProductList, "3", "abc123"); err == nil {
...
func TestStripProductPage_Contract(t *testing.T) {
	...
	stripped := StripProductPage([]byte(live))
	if stripped == nil {
...
func TestStripProductPage_RejectsGarbage(t *testing.T) {
	if StripProductPage([]byte(`{not json`)) != nil {
		t.Fatal("garbage must strip to nil")
	}
	if StripProductPage(nil) != nil {

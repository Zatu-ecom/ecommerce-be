...
type BundleConfig struct {
	...
	BundleDiscountType  DiscountType       `json:"bundle_discount_type"            binding:"required,oneof=percentage fixed_amount fixed_price"`
...
const (
	...
	DiscountTypeFixedPrice  DiscountType = "fixed_price" // bundle only
)

type TierType string

const (
	TierTypeQuantity TierType = "quantity"
	TierTypeSpend    TierType = "spend"
)

type TierConfig struct {
	Min           int          `json:"min"            binding:"required,min=0"`
	Max           *int         `json:"max,omitempty"  binding:"omitempty,min=1"`
	DiscountType  DiscountType `json:"discount_type"  binding:"required,oneof=percentage fixed_amount"`
...
type TieredConfig struct {
	TierType TierType     `json:"tier_type" binding:"required,oneof=quantity spend"`

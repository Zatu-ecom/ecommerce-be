...
CREATE TABLE IF NOT EXISTS storage_config (
    ...
    -- PLATFORM rows store the acting admin user id; SELLER rows store the seller user id.
...
-- At most one default config per seller (owner_id).
CREATE UNIQUE INDEX IF NOT EXISTS uq_storage_config_seller_default
    ON storage_config(owner_id)
    WHERE owner_type = 'SELLER' AND is_default = true;

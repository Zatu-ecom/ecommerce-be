-- Migration: 030_money_currency_standardization.sql
-- Description: Rename catalog price columns (product_variant.price, package_option.price)
-- from DOUBLE PRECISION major units to BIGINT minor units (price_cents), backfill existing
-- values assuming two-decimal historical prices, and update the related-products function
-- to consume price_cents.
-- Created: 2026-08-02
-- Feature: 008-money-currency-standardization

-- ============================================================================
-- 1. product_variant: price -> price_cents
-- ============================================================================

-- Add nullable column first so backfill can run without blocking.
ALTER TABLE product_variant ADD COLUMN IF NOT EXISTS price_cents BIGINT;

-- Backfill from the existing decimal major-unit column.
-- Historical prices are assumed two-decimal (ROUND(price * 100)); an audit of
-- non-2dp sellers must be run before production rollout per the feature spec.
UPDATE product_variant
SET price_cents = ROUND(price * 100)::BIGINT
WHERE price_cents IS NULL;

-- Enforce NOT NULL now that all rows are backfilled.
ALTER TABLE product_variant ALTER COLUMN price_cents SET NOT NULL;

-- Drop the old ambiguous column and replace its index.
DROP INDEX IF EXISTS idx_variant_product_price;
ALTER TABLE product_variant DROP COLUMN IF EXISTS price;
CREATE INDEX IF NOT EXISTS idx_variant_product_price_cents ON product_variant(product_id, price_cents);

-- ============================================================================
-- 2. package_option: price -> price_cents
-- ============================================================================

ALTER TABLE package_option ADD COLUMN IF NOT EXISTS price_cents BIGINT;

UPDATE package_option
SET price_cents = ROUND(price * 100)::BIGINT
WHERE price_cents IS NULL;

ALTER TABLE package_option ALTER COLUMN price_cents SET NOT NULL;

ALTER TABLE package_option DROP COLUMN IF EXISTS price;
CREATE INDEX IF NOT EXISTS idx_package_option_product_price_cents ON package_option(product_id, price_cents);

-- ============================================================================
-- 3. Related-products function: consume price_cents instead of price
-- ============================================================================

-- Drop the old functions (their signatures reference price semantics implicitly;
-- recreation below uses price_cents).
...
-- Recreate the scored function. All price math is integer minor units (price_cents);
-- returned min_price/max_price remain major-unit floats for API compatibility and are
-- derived via division by 100 (two-decimal assumption consistent with backfill).
...
DECLARE
    ...
    v_source_tags TEXT[];
    v_source_min_price NUMERIC(14,2);
    v_source_max_price NUMERIC(14,2);
...
BEGIN
    ...
    price_range AS (
        ...
        FROM product p
        INNER JOIN (SELECT pv.product_id, MIN(pv.price_cents) as min_price, MAX(pv.price_cents) as max_price
    ...
    scored_products AS (
        ...
        LEFT JOIN category c ON p.category_id = c.id
        LEFT JOIN (SELECT pv.product_id, MIN(pv.price_cents) as min_price, MAX(pv.price_cents) as max_price, BOOL_OR(pv.allow_purchase) as allow_purchase, COUNT(*) as total_variants, COUNT(*) as in_stock_variants FROM product_variant pv GROUP BY pv.product_id) pv ON p.id = pv.product_id
    ),
    ...
    SELECT p.id, p.name, p.category_id, c.name, c.parent_id, pc.name, p.brand, p.base_sku AS sku,
           ...
           COALESCE((pv.min_price / 100.0)::DOUBLE PRECISION, 0.0),
           COALESCE((pv.max_price / 100.0)::DOUBLE PRECISION, 0.0),
    ...
    LEFT JOIN (SELECT v.product_id, MIN(v.price_cents) as min_price, MAX(v.price_cents) as max_price, BOOL_OR(v.allow_purchase) as allow_purchase, COUNT(*) as total_variants, COUNT(*) as in_stock_variants FROM product_variant v GROUP BY v.product_id) pv ON p.id = pv.product_id
...
ANALYZE package_option;

COMMENT ON FUNCTION get_related_products_scored IS 'Retrieves related products using 8 strategies, priced via price_cents (minor units).';

...
func (h *AttributeHandler) UpdateAttribute(c *gin.Context) {
	...
	if err != nil {
		common.ErrorWithCode(c, http.StatusBadRequest, "Invalid attribute ID", utils.VALIDATION_ERROR_CODE)
		return
	}
	...
	if err != nil {
		if err.Error() == utils.ATTRIBUTE_DEFINITION_NOT_FOUND_MSG {
			common.ErrorWithCode(c, http.StatusNotFound, err.Error(), utils.ATTRIBUTE_DEFINITION_NOT_FOUND_CODE)
			return
		}
		if err.Error() == utils.ATTRIBUTE_DATA_TYPE_INVALID_MSG {
			common.ErrorWithCode(c, http.StatusBadRequest, err.Error(), utils.ATTRIBUTE_DATA_TYPE_INVALID_CODE)
		...
		common.ErrorResp(c, http.StatusInternalServerError, utils.FAILED_TO_UPDATE_ATTRIBUTE_MSG+": "+err.Error())
		return
	}
...
func (h *AttributeHandler) GetAllAttributes(c *gin.Context) {
	...
	if err != nil {
		common.ErrorResp(c, http.StatusInternalServerError, utils.FAILED_TO_GET_ATTRIBUTES_MSG+": "+err.Error())
		return
	}

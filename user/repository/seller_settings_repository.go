...
func (r *SellerSettingsRepositoryImpl) Create(ctx context.Context, settings *entity.SellerSettings) error {
	return db.DB(ctx).Create(settings).Error
}

// FindBySellerID retrieves seller settings by seller ID
func (r *SellerSettingsRepositoryImpl) FindBySellerID(ctx context.Context, sellerID uint) (*entity.SellerSettings, error) {
...
func (r *SellerSettingsRepositoryImpl) Update(ctx context.Context, settings *entity.SellerSettings) error {
	return db.DB(ctx).Save(settings).Error
}

// ExistsBySellerID checks if seller settings exist for a given seller ID
func (r *SellerSettingsRepositoryImpl) ExistsBySellerID(ctx context.Context, sellerID uint) (bool, error) {
	var count int64
	err := db.DB(ctx).Model(&entity.SellerSettings{}).Where("seller_id = ?", sellerID).Count(&count).Error
	if err != nil {
		return false, err
	}
	return count > 0, nil
